...
import PhotoUpload from "./PhotoUpload";
import { updatePhotoUrl } from "../auth/authSlice";
...
export default function ProfilePhotos({ profile }: Props) {
  ...
  const { loadCollection, deleteDocument } = useFireStore(
  ...
  async function handleSetMain(photo: Photo) {
    await updateDocument(profile.id, {
      photoURL: photo.url,
    });
    await updateProfile(auth.currentUser!, {
      photoURL: photo.url,
    });
    dispatch(updatePhotoUrl(photo.url));
  }
